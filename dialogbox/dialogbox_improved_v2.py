...
class DirDialogBox:
    ...
    def open_folder(
        ...
        if path is None and DialogBoxConfig.remember_paths:
            start_path = DialogBoxConfig.last_dir_path
        elif path is None:
...
class FileDialogBox:
    ...
    def open_file(
        ...
        # 시작 경로 결정
        if path is None and DialogBoxConfig.remember_paths:
            start_path = DialogBoxConfig.last_file_path
        elif path is None:
            start_path = Path.home()
        else:
            start_path = Path(path)
        ...
        dlg.setDirectory(str(start_path))
    ...
    def open_files(
        ...
        if dlg.exec():
            selected = dlg.selectedFiles()
            if selected:
                results = [Path(f) for f in selected]
                ...
                if validate:
                    ...
                    for result in results:
                        try:
                            ...
                            invalid_count += 1
                            logger.error(f"Validation error for {result}: {e}")
...
class CommonFilters:
    ...
    def custom(*extensions: str) -> str:
        ...
        Example:
            filter = CommonFilters.custom("log", "txt", "out")
            # "Files (*.log *.txt *.out);;All Files (*)"
        """
