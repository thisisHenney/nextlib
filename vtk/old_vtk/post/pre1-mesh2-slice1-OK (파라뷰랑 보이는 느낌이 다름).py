...
class FoamMeshViewer(QWidget):
    ...
    def update_slice(self):
        """슬라이스 방향/위치/Clip 옵션에 따라 VTK 파이프라인 업데이트."""
        ...
        # ==================================================================
        # 1) Slice (Cutter)
        #    reader → Cutter → CompositeDataGeometry → PolyData → Actor
        # ==================================================================
        # ==================================================================
        ...
        # 2. Cell → Point interpolation (ParaView와 동일)
        ...
        # ==================================================================
        # 2) Clip (옵션) - ParaView처럼 Slice + Clip 조합
        #    reader → ClipDataSet → CompositeDataGeometry → PolyData → Actor
        # ==================================================================
        if self.chk_clip.isChecked():
            # surface_actor는 숨기고 클립된 외곽만 보여줌
            if self.surface_actor is not None:
                self.surface_actor.SetVisibility(False)

            if self.clip_filter is None:
                self.clip_filter = vtk.vtkClipDataSet()
                self.clip_filter.SetInputConnection(self.foam_reader.GetOutputPort())
                self.clip_filter.SetClipFunction(self.slice_plane)
                # InsideOutOn: 평면 한쪽을 남기는 방향 (필요시 On/Off 바꾸면 방향 반대)
                self.clip_filter.InsideOutOn()

                self.clip_geom = vtk.vtkCompositeDataGeometryFilter()
                self.clip_geom.SetInputConnection(self.clip_filter.GetOutputPort())

                self.clip_mapper = vtk.vtkPolyDataMapper()
                self.clip_mapper.SetInputConnection(self.clip_geom.GetOutputPort())
            ...
            # Clip 사용 안 하면, 클립 actor 제거 & 원래 surface 표시
            if self.clip_actor is not None:
                self.renderer.RemoveActor(self.clip_actor)
                self.clip_actor = None
                self.clip_filter = None
        ...
        # 파이프라인 업데이트
        if self.slice_cutter is not None:
            self.slice_cutter.Update()
