...
def write_list_file(file_path: Path | str, data: Sequence[str]) -> bool:
    p = Path(file_path)
...
def append_file(file_path: Path | str, data: str, add_return: bool = False) -> bool:
    ...
    with p.open('a', encoding=encoding_type) as f:
        if add_return:
            f.write('\n')
        f.write(data)
    return True
...
def copy_or_move_items(src: str | Path, dest: str | Path,
                       pattern: str = "*", action: str = 'copy') -> None:
    ...
    for root, _, files in os.walk(src):
        ...
        for file_name in files:
            if fnmatch.fnmatch(file_name, pattern):
                op(root_path / file_name, dest_path / file_name)
...
def get_temp_dir(base_dir: str, prefix: str = "NewTempCase", width: int = 3) -> str:
